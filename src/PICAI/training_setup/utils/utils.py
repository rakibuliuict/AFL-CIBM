...
def count_foreground_background(label):
    if not isinstance(label, torch.Tensor):
        raise ValueError("Label must be a PyTorch tensor.")
...
def binarize_output(logits, threshold=0.5):
    binary_mask = (logits >= threshold).float()
    return binary_mask
...
def calculate_mean_smoothness(gradient_magnitude):
    ...
    mean_smoothness = np.mean(gradient_magnitude)
...
def compute_smoothness(image_tensor):
    ...
    mean_smoothness = calculate_mean_smoothness(gradient_magnitude)
